...
class Pixiv:
    ...
    def get_login_post_key(self):
        ...
        if login_page_response.status_code == 200:
            ...
            return matched, login_page_response.cookies
        else:
            return None, None

    def login(self, username, password):
        post_key, cookies = self.get_login_post_key()
        if post_key is not None:
            ...
            res = self.core.post(url=self.urls["login"], data=data, cookies=cookies, headers=headers, rand_user_agent=False)
        ...
        if res.status_code == 200:
            ...
            if bs is not None:
                try:
                    ...
                    user = Pixiv.get_bs_by_re(bs, 'script', r'"userName":"([^"]*)"')[0].encode('utf-8').decode('unicode-escape')
                ...
                while page_looping and (count is None or index < count):
                    use_count = 3
                    img_url, pg_index, extension = self.format_original(original_src, index)
                    res_img = self.core.get(url=img_url, headers={'Referer': self.urls["illust"] % (pid,)})
                    while use_count > 0:
                        if res_img.status_code == 404:
                            page_looping = False
                            break
    ...
    def format_original(self, src, index=None):
        res = re.search(r'img\\/([0-9]*)\\/([0-9]*)\\/([0-9]*)\\/([0-9]*)\\/([0-9]*)\\/([0-9]*)\\/([0-9]*)_p([0-9]*).(\w*)', src).groups()
        return (
